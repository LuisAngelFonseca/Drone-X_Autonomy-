...
def display_override_text(frame_equ):
    ...
    if 1 < actual_time - elapsed_override_blink < 2:
        cv2.putText(frame_equ, text='OVERRIDE MODE: ON', org=(5, 710), fontFace=font, fontScale=1,
                    color=(0, 0, 255),
                    thickness=2, lineType=cv2.LINE_8)
...
class DroneX(object):
    ...
    def run(self):
        ...
        while drone_continuous_cycle:
            ...
            frame_user = self.display_battery(display_text(frame_grid))
            if OVERRIDE:
            ...
            if self.is_flying_send_rc_control and not OVERRIDE and not args.debug:
                ...
                if detection:
                    self.yaw_velocity, self.up_down_velocity, self.for_back_velocity = drone_stay_close(x, y, x_1,
                                                                                                        x_2, y_1,
                                                                                                        y_2, r,
                                                                                                        radius_stop,
    ...
    def display_battery(self, frame_equ):
